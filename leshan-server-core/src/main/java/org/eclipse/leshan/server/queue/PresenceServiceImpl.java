...
/**
 * Tracks the status of each LWM2M client registered with Queue mode binding.
 * Also ensures that the {@link PresenceListener} are notified on state changes
 * only for those LWM2M clients registered using Queue mode binding.
 * 
 * @see Presence
 */
public final class PresenceServiceImpl implements PresenceService {
	...
	/**
	 * Set the state of the client identified by registration as
	 * {@link Presence#AWAKE}
	 * 
	 * @param reg
	 *            the client's registration object
	 */
	...
	/**
	 * Notify the listeners that the client state changed to
	 * {@link Presence#SLEEPING}. The state changes is produced inside
	 * {@link PresenceStatus} when the timer expires or when the client doesn't
	...
	/**
	 * Removes the {@link PresenceStatus} object associated with the client from the
	 * list.
	 * 
	 * @param reg
	 *            the client's registration object.
	...
	/**
	 * Returns the {@link PresenceStatus} object associated with the given endpoint
	 * name.
	 * 
	 * @param reg
	 *            The client's registration object.
	...
	/**
	 * Start or restart (if already started) the timer that handles the client wait
	 * before sleep time.
	 * 
	 * @param status
	 */
	...
	/**
	 * Called when the client doesn't respond to a request, for changing its state
	 * to SLEEPING
	 */

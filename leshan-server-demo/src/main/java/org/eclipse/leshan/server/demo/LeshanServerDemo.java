...
import org.eclipse.leshan.server.queue.QueueModeObjectClientAwakeTimeProvider;
...
public class LeshanServerDemo {
	...
	public static void createAndStartServer(int webPort, String localAddress, int localPort, String secureLocalAddress,
		...
		lwServer.getPresenceService().addListener(new TestQueueModeListener(lwServer));
		lwServer.setClientAwakeTimeProvider(new QueueModeObjectClientAwakeTimeProvider(lwServer));
		// Start Jetty & Leshan
		lwServer.start();
		server.start();
